...
def generate_egonet(clust_selection, SM_MS2DEEPSCORE, TSNE_DF):
    ...
    if len(clust_selection) > 1:
        ...
        print("Warning: extracted first node for ego net construction; ", clust_selection)
    # Define Network Data
    ...
    print(len(nodes))
    active_style_sheet = [{'selector' : 'node', 'style' : {'height' : "100%", 'width' : '100%', 'opacity' : 0.6}}]
    elements = nodes # initialize elements to just the nodes
    ego = str(clust_selection[0])
    bdic = utils054.creating_branching_dict(ego, edge_dict, node_dict, n_levels = 5)
    ...
    active_style_sheet = active_style_sheet + tmp_edge_styles + [{"selector": 'node[id= "{}"]'.format(ego), 
        "style": {"shape": "diamond",'background-color': 'gold' ,'opacity': 0.95, 'height': '105%', 'width' : '105%'}}]
