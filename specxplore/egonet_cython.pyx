...
def creating_branching_dict(
    ...
    cdef signed long long n_edges = np.int64(source.shape[0])
    cdef set tmp_nodes
    cdef set tmp_edges
    ...
    tmp_nodes = set()
    tmp_edges = set()
    for index in np.arange(zero, n_edges, 1, dtype=np.int64):
    ...
    for index in np.arange(1, n_levels, 1, dtype = np.int64):
        ...
        for inner_index in np.arange(0, n_edges, 1, dtype=np.int64):
    ...
    if np.int64(len(tmp_nodes)) != np.int64(0):
    ...
    cdef signed long long edge_counter = np.int64(0)
    cdef signed long long edges_omitted = np.int64(0)
    cdef signed long long new_edges
    cdef signed long long allowed_new_edges
    for level in branching_dict.keys():
        new_edges = np.int64(len(branching_dict[level]["edges"]))
        if edge_counter + len(branching_dict[level]["edges"]) > max_edges:
            n_new_edges = np.int64(len(branching_dict[level]["edges"]))
...
def generate_edge_elements_and_styles(
    ...
    for _, key in enumerate(branching_dict):
